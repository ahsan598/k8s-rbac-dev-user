# Kubernetes RBAC: Namespace-Level Admin Access
...
- CSR + Certificate → *Authentication* (Who are you?)
- RBAC (Role / RoleBinding) → *Authorization* (What can you do?)
...
├── certs/                     # Certificate-based authentication (DO NOT COMMIT)
│   ├── dev.key                # Private key for dev-user
│   ├── dev.csr                # Certificate Signing Request submitted to Kubernetes
│   └── dev.crt                # Signed client certificate issued by Kubernetes CA
├── rbac/                      # RBAC authorization definitions
│   ├── role-dev-admin.yaml    # Namespace-scoped admin Role for dev namespace
│   └── rolebinding-dev-admin.yaml # Binds dev-user to dev-admin-role
├── dev-user-csr.yaml          # Kubernetes CSR resource for dev-user authentication
├── namespace.yaml             # Creates and scopes the dev namespace
├── .gitignore                 # Excludes sensitive certs and local artifacts
└── readme.md                  # Project documentation explaining auth, RBAC, and flow
```
...
### 📝 Key Takeaways
- RBAC does not create users — it only authorizes identities 
